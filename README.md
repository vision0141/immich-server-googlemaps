...
- アセット詳細パネルの外部マップリンクを `OpenStreetMap` から `Google Maps` に変更
  表示は `place/{DMS}/@{lat},{lon},120m` 形式で、地点周辺を見やすい距離から開く
...
- OpenStreetMap URL -> Google Maps URL (`place/{DMS}/@{lat},{lon},120m`)
- リンク表示名 -> `Google Maps`
...
- changing the asset detail panel's external map link from OpenStreetMap to Google Maps
  using a `place/{DMS}/@{lat},{lon},120m` style URL for a closer initial view
